...
IF OBJECT_ID('p_get_Rate_ForApi_Dashboardv2', 'P') IS NOT NULL
   DROP PROCEDURE [dbo].p_get_Rate_ForApi_Dashboardv2
GO


CREATE PROCEDURE [dbo].p_get_Rate_ForApi_Dashboardv2  
AS  
BEGIN  
...
--@modified on April 25, 2016
--Added new payouts
--------------------------------------------  
  
            DECLARE @t table ( country varchar(20), cntry varchar(20), flagName varchar(50), unit varchar(5));  
            ...
            INSERT INTO @t (country,cntry,flagName, unit) values('BANGLADESH' ,'Bangladesh' ,'Bangladesh.png','1'),  
																  ...
																  ('UNITED STATES' ,'United States' ,'United States of America (USA).png','1'),
																  ('UNITED KINGDOM' ,'United Kingdom' ,'United Kingdom(Great Britain).png','1'),
																  ('NEW ZEALAND' ,'New Zealand' ,'New Zealand.png','1'),
																  ('AUSTRALIA' ,'Australia' ,'Australia.png','1');
      ...
      SELECT     f1.grp_cd
			...
			-- ,f1.created_on
			 ...
			 ,ROW_NUMBER() OVER(partition BY  sub_agent_cd,  c1.cntry_name ORDER BY f1.modified_on DESC) AS row_rank
				FROM dbo.grp_forex f1 WITH (NOLOCK)
